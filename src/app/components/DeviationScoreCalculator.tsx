...
const calculateAdjustedDeviationScore = (
  ...
  const baseDeviationScore = (
    ...
    const avgDaily =
      (avgWeekday[grade][subject] * 5 + avgWeekend[grade][subject] * 2) / 7;
    const personalDaily =
      ((weekdayTime.hours * 60 + weekdayTime.minutes) * 5 +
        (weekendTime.hours * 60 + weekendTime.minutes) * 2) /
      7;

    const deviation = 50 + (10 * (personalDaily - avgDaily)) / stdDev[grade];
    return deviation;
  };

  const adjustedPreviousScore =
    previousScore !== undefined ? previousScore : 50;

  const currentBaseScore = baseDeviationScore(
    currentGrade,
    subject,
    weekdayTime,
    weekendTime
  );
  const diff = Math.abs(currentBaseScore - adjustedPreviousScore);
  const dampeningFactor = 1 / (1 + 0.01 * diff);
  const adjustedScore =
    currentBaseScore * dampeningFactor +
    adjustedPreviousScore * (1 - dampeningFactor);

  const finalScore = Math.max(30, Math.min(80, adjustedScore));
...
const DeviationScoreCalculator: React.FC = () => {
  ...
  const [results, setResults] = useState<{ [key in Subjects]: number }>({
    国語: 0,
    数学: 0,
    理科: 0,
    社会: 0,
  ...
  const handlePreviousScoreChange = (
    e: ChangeEvent<HTMLInputElement>,
    subject: Subjects
  ) => {
    const value = Number(e.target.value);
        ...
        <div className="grade-select">
          ...
          <select
            ...
            onChange={(e: ChangeEvent<HTMLSelectElement>) =>
              setCurrentGrade(Number(e.target.value) as Grade)
            }
          >
